...
\title{
    \vspace{-1cm}
    \textbf{Postera: A Fully Quantum-Resistant Privacy Cryptocurrency}\\[0.5em]
...
\begin{abstract}
We present Postera v2, a cryptocurrency providing strong privacy guarantees that remain secure against both classical and quantum adversaries. Unlike previous versions that used quantum-vulnerable pairing-based proofs for privacy, Postera v2 achieves \textbf{full post-quantum security} by combining NIST-standardized signatures (ML-DSA-65, FIPS 204) with hash-based STARK proofs (Plonky2 over the Goldilocks field). This eliminates all reliance on elliptic curve discrete logarithm assumptions, providing complete protection against Shor's algorithm. The protocol maintains a note-based UTXO model similar to Zcash's Sapling but replaces all quantum-vulnerable components with lattice-based and hash-based alternatives. This paper describes the cryptographic foundations, V2 transaction model, migration path from V1, and security properties of the fully quantum-resistant Postera protocol.
\end{abstract}
...
While Postera v1 addressed signature security with ML-DSA-65, the privacy layer remained vulnerable due to its use of BN254-based Groth16 proofs and Pedersen commitments.

\subsection{Postera v2: Full Quantum Resistance}

Postera v2 eliminates all quantum-vulnerable components:
...
ZK Proofs & Groth16/BN254 & Plonky2 STARKs & \textcolor{quantumgreen}{\checkmark} \\
...
Postera uses ML-DSA-65 (Module-Lattice Digital Signature Algorithm), standardized as FIPS 204 by NIST.
...
\subsection{Post-Quantum Zero-Knowledge Proofs: Plonky2 STARKs}

Postera v2 replaces Groth16 with Plonky2, a STARK-based proving system that relies only on hash function security.
...
\subsubsection{Plonky2 Parameters}
...
\bottomrule
\end{tabular}
\caption{Plonky2 configuration in Postera v2}
\end{table}

\subsubsection{Why Plonky2?}
...
The hash outputs 4 field elements (256 bits total), matching Plonky2's internal representation.
...
ShieldedTransactionV2 {
    ...
    fee: u64,
    transaction_proof: {
        proof_bytes: Vec<u8>,  // Plonky2 STARK (~50KB)
        public_inputs: {
            merkle_roots: [[u8; 32]],
            nullifiers: [[u8; 32]],
...
Postera V2 transactions are secure against adversaries with access to cryptographically-relevant quantum computers.
\end{theorem}
...
\begin{enumerate}
    ...
    \item \textbf{Plonky2 STARKs}: Security reduces to hash collision resistance
\end{enumerate}
...
The larger proof size is an acceptable trade-off for quantum resistance. Proof generation is actually \textit{faster} in V2 due to Plonky2's optimized WASM implementation.
...
The Plonky2 prover is compiled to WebAssembly for client-side execution:
...
Proof verification uses native Rust Plonky2:
...
Postera v2 achieves \textbf{complete post-quantum security} for private cryptocurrency transactions. By replacing all elliptic curve-based components with hash-based alternatives:

\begin{itemize}
    \item \textbf{Signatures}: ML-DSA-65 (lattice-based, NIST standardized)
    \item \textbf{ZK Proofs}: Plonky2 STARKs (hash-based, no trusted setup)
...
We thank the developers of Plonky2 (Polygon Zero), the NIST PQC standardization team, and the Zcash protocol designers whose work inspired Postera's privacy model.
